...
curl_setopt($ch1, CURLOPT_URL, "https://api.recetasalud.ar/colfacor/v2/api/dispensa");
...
mysqli_query($conexion,"INSERT INTO recetas(id_obrasocial, num_receta, fecha_receta, mp_med, dni_afi, estado, tipo, dia, mes, ano, hora, minuto, ipfarm, fecha_alta, linkreceta)
    VALUES ('$id_obrasocial', '$num_receta',  '$fecha_emision_subst', '$matricprescr', '$dni', '1', '1', '$dia', '$mes', '$ano', '$hora', '$minuto', '$ip_add', '$fecha_hoy', '$linkreceta')");
...
fwrite($logFile, "\n".date("d/m/Y H:i:s")." INICIO ANULAR: ".$cuit.' / '.$idsucursal.' - '.$ip_add.' - '.$num_validacion.' - '.$num_receta.' - '.$fecha_hoy.' '.$hora.':'.$minuto) or die("Error escribiendo en el archivo");fclose($logFile);
include ('inc/conexion.php');
date_default_timezone_set('America/Argentina/Buenos_Aires');
$fecha_hoy = date("Y-m-d");
$ano = date('Y');
...
$ip_add = $_SERVER['REMOTE_ADDR'];
$logFile = fopen("../log/log_anulaciones.txt", 'a') or die("Error creando archivo");
fwrite($logFile, "\n".date("d/m/Y H:i:s")." Busco en la base: ".$cuit.' / '.$idsucursal.' - '.$ip_add.' - '.$num_validacion.' - '.$num_receta.' - '.$fecha_hoy.' '.$hora.':'.$minuto) or die("Error escribiendo en el archivo");fclose($logFile);
...
$total = $rs3['total'];
$logFile = fopen("../log/log_anulaciones.txt", 'a') or die("Error creando archivo");
fwrite($logFile, "\n".date("d/m/Y H:i:s")." INSERTO validaciones_anuladas: ".$cuit.' / '.$idsucursal.' - '.$ip_add.' - '.$num_validacion.' - '.$num_receta.' - '.$fecha_hoy.' '.$hora.':'.$minuto) or die("Error escribiendo en el archivo");fclose($logFile);
...
$logFile = fopen("../log/log_anulaciones.txt", 'a') or die("Error creando archivo");
fwrite($logFile, "\n".date("d/m/Y H:i:s")." Le pego al endpoint: ".$cuit.' / '.$idsucursal.' - '.$ip_add.' - '.$num_validacion.' - '.$num_receta.' - '.$fecha_hoy.' '.$hora.':'.$minuto) or die("Error escribiendo en el archivo");fclose($logFile);
$ch = curl_init(); // Ini
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    "Content-Type:application/json"
));
...
$row=json_decode($respuesta);
$logFile = fopen("../log/log_anulaciones.txt", 'a') or die("Error creando archivo");
fwrite($logFile, "\n".date("d/m/Y H:i:s")." me responde el endpoint:  " .$respuesta. ' / '.$cuit.' / '.$idsucursal.' - '.$ip_add.' - '.$num_validacion.' - '.$num_receta.' - '.$fecha_hoy.' '.$hora.':'.$minuto) or die("Error escribiendo en el archivo");fclose($logFile);
$logFile = fopen("../log/log_anulaciones.txt", 'a') or die("Error creando archivo");
fwrite($logFile, "\n".date("d/m/Y H:i:s")." Deletes: ".$cuit.' / '.$idsucursal.' - '.$ip_add.' - '.$num_validacion.' - '.$num_receta.' - '.$fecha_hoy.' '.$hora.':'.$minuto) or die("Error escribiendo en el archivo");fclose($logFile);
...
$logFile = fopen("../log/log_anulaciones.txt", 'a') or die("Error creando archivo");
fwrite($logFile, "\n".date("d/m/Y H:i:s")." Fin deletes: ".$cuit.' / '.$idsucursal.' - '.$ip_add.' - '.$num_validacion.' - '.$num_receta.' - '.$fecha_hoy.' '.$hora.':'.$minuto) or die("Error escribiendo en el archivo");fclose($logFile);

fwrite($logFile, "\n".date("d/m/Y H:i:s")." Fin Anulacion: ".$cuit.' / '.$idsucursal.' - '.$ip_add.' - '.$num_validacion.' - '.$num_receta.' - '.$fecha_hoy.' '.$hora.':'.$minuto) or die("Error escribiendo en el archivo");fclose($logFile);
//echo header("location: validaciones.php?msj=".base64_encode('an')."");
mysqli_close($conexion);
//return base64_encode('an');
return "validaciones.php?msj=".base64_encode('an')."";
}
?>