...
$estado = $rs1['estado'];
$fecha_inicio = $rs1['fecha_inicio'];
$fecha_bloqueo = $rs1['fecha_bloqueo'];
$periodo = $rs1['periodo'];
?>
<div class="card">
	...
	<div class="card-body">
	  ...
	  <div class="alert alert-warning alert-styled-left alert-arrow-left alert-dismissible">
		...
		<span class="font-weight-semibold">RECUERDE!</span> El Cierre PROGRAMA MUNICIPALIDAD MEJORAR se debe realizar en la SEGUNDA QUINCENA.
    </div>	
		<?php 
    ...
    if(base64_decode($_GET['msj']) == 'rep'){
	echo'
	<div class="alert alert-success alert-styled-left alert-arrow-left alert-dismissible">
		<button type="button" class="close" data-dismiss="alert"><span>&times;</span></button>
		<span class="font-weight-semibold">ATENCION!</span> Ya tiene realizado un cierre en este periodo.
    </div>'; } ?>
 
    	<?php 
    		if($fecha_hoy >= $fecha_inicio AND $fecha_hoy < $fecha_bloqueo AND $estado == 1){
    		echo'
		<a href="cierre_os_seleccionar.php"><button type="button" id="" class="btn btn-danger"> Realizar Cierre</button></a><br><br>';
		...
		<div class="table-responsive">
			...
			<thead>
			    ...
			    <?php
			    include ('inc/conexion.php');
	            $registros = mysqli_query($conexion," SELECT a.periodo, a.num_cierre, a.cant_recetas, a.fecha_alta, d.obra_social
	            										...
	            										WHERE c.cuit_farm = '$cuit' and c.suc_farm = '$idsucursal' 
	            										GROUP BY a.periodo, a.num_cierre, a.cant_recetas, a.fecha_alta, d.obra_social
	            										ORDER BY a.periodo DESC");
	           	while ($reg=mysqli_fetch_array($registros)){  
	           		if($periodo <> $reg['periodo']){
	        		echo "<tr>"
	                ."<td>".$reg['obra_social']."</td>"
	                ."<td>".$reg['periodo']."</td>"
					...
					<a href='cierres_detalle.php?num_cierre=".base64_encode($reg['num_cierre'])."'><button type='button' class='btn bg-primary-400 btn-icon '><i class='icon-list2'></i></button></a>
					</td>"
